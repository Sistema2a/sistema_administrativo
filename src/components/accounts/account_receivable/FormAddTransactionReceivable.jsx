...
const FormAddTransactionReceivable = ({ accountsReceivable }) => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    const res = await axios.post(
      "/api/accounts/account_receivable/history_transactions",
      data
    );
                ...
                <div className="relative mt-1">
                  ...
                  >
                    {accountsReceivable.map((account, index) => (
                      <option
                        key={index}
                        value={account.users_account_receivable_id}
                      >
                        {account.account}
                      </option>
                    ))}
              ...
              <div>
                ...
                <div className="relative mt-1">
                  <select
                    ...
                    className="block w-full pl-4 pr-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                  >
                    <option value="cobro_deuda">Cobrar de deuda</option>
                    <option value="cuenta_cobrar">Cuenta por cobrar</option>
                  </select>
                </div>
                {errors.concept && (
                  <p className="text-red-500 text-sm mt-1">
...
export default FormAddTransactionReceivable;
